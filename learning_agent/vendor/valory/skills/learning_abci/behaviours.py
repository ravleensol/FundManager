...
class DecisionMakingBehaviour(LearningBaseBehaviour):  # pylint: disable=too-many-ancestors
    ...
    def make_transaction_decision(self) -> Generator[None, None, str]:
        """Download proposals from IPFS, sort them, and make a decision."""
        ...
        for proposal in proposals:
            proposal_amount = int(proposal[1])
            self.context.logger.info(f"Proposal amount: {proposal_amount}")
            if (
                self.params.min_proposal_amount
                < proposal_amount
                < self.params.max_proposal_amount
            ):
                
                return Event.TRANSACT.value, {
                    "proposal_amount": proposal[1],
                }
            else:
                self.context.logger.info(
                    f"Proposal amount {proposal_amount} is outside the range"
                )

        self.context.logger.info(
            "No proposals within the range; deciding to HOLD"
        )
...
class TxPreparationBehaviour(
    ...
    def _build_execute_txn(self) -> Generator[None, None, Optional[bytes]]:
        response = yield from self.get_contract_api_response(
            performative=ContractApiMessage.Performative.GET_STATE,  # type: ignore
            contract_id=str(FundManager.contract_id),
            contract_callable="build_execute_proposal_tx",
            contract_address=self.params.fund_manager_contract_address,
            amount=self.synchronized_data.proposal_amount
        )
    ...
    def _get_multisend_tx(self, txs: List[bytes]) -> Generator[None, None, Optional[str]]:
        ...
        multi_send_approve_tx = self._to_multisend_format(txs[0], self.params.token)
